...
開啟 `admin.html` 可以瀏覽、編輯、上傳或取代 repository 內的檔案。管理頁預設連到 `Axoled-Student/Baechhhh` 的 `main`，每次寫入都會建立 GitHub commit。

基於安全原因，Personal Access Token 不會寫在 HTML 或 JavaScript，也不會提交到 GitHub。請使用只授權此 repository、具備 `Contents: Read and write` 的 fine-grained token，在管理頁開啟時貼上。Token 只保存在該分頁的 `sessionStorage`，關閉分頁或按下登出後即清除。
