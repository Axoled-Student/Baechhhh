...
網站第一次在 iPad Safari 開啟時，會在背景把三支影片存進裝置快取。固定展示期間不需要觸控操作；網頁會定期檢查網站更新，並每六小時在背景重新確認影片快取。

## 放置感應板塊

貴賓畫面只顯示三個簡單步驟：選一塊板塊、平放感應區、欣賞自動播放的影片。技術上，程式是依 GPIO 34 的 ADC 值辨識板塊；若數值落在三個有效範圍以外，網站會回到等待畫面。
